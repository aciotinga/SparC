...
- `dataset`: 2D `np.ndarray` of shape `(n_samples, n_columns)`. Discrete
  circuits use integer outcomes (`dtype=int32`); continuous Gaussian circuits
  use `float64` (`NaN` = missing). Column `j` holds values for variable `j`
  by default.
