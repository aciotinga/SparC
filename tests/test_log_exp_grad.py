...
class TestLogExpForwardParity:
    def test_matches_log_of_exp(self):
        c1a = CategoricalInputNode(scope_var=0, probabilities=[0.7, 0.3])
        c1b = CategoricalInputNode(scope_var=0, probabilities=[0.2, 0.8])
        circ1 = SumNode(children=[c1a, c1b], parameters=[0.4, 0.6])
        c2a = CategoricalInputNode(scope_var=0, probabilities=[0.5, 0.5])
        c2b = CategoricalInputNode(scope_var=0, probabilities=[0.1, 0.9])
        circ2 = SumNode(children=[c2a, c2b], parameters=[0.55, 0.45])
...
class TestLogExpGradients:
    def test_leaf_grad_circuit2_matches_fd(self):
        ...
        def f(q):
            leaf2 = CategoricalInputNode(scope_var=0, probabilities=list(q))
    ...
    def test_leaf_grad_circuit1_matches_fd(self):
        ...
        def f(p):
            leaf1 = CategoricalInputNode(scope_var=0, probabilities=list(p))
            return log_exp_query(leaf1, leaf2)

        leaf1 = CategoricalInputNode(scope_var=0, probabilities=list(p_init))
        _, g1, _ = log_exp_query_and_grad(leaf1, leaf2)
        g_an = project_to_simplex_tangent(g1.cat_grads[int(leaf1.id)])
    ...
    def test_sum_sum_grad_matches_fd(self):
        ...
        def f(phi):
            c2a = CategoricalInputNode(scope_var=0, probabilities=[0.5, 0.5])
            c2b = CategoricalInputNode(scope_var=0, probabilities=[0.1, 0.9])
            circ2 = SumNode(children=[c2a, c2b], parameters=list(phi))
            return log_exp_query(circ1, circ2)

        c2a = CategoricalInputNode(scope_var=0, probabilities=[0.5, 0.5])
        c2b = CategoricalInputNode(scope_var=0, probabilities=[0.1, 0.9])
        circ2 = SumNode(children=[c2a, c2b], parameters=list(q_init))
    ...
    def test_product_grad_matches_fd(self):
        ...
        def f(probs):
            ...
            return log_exp_query(circ1, ProductNode(children=[c2a, c2b]))
