...
class TestBernoulliLikelihoodGrad:
    def test_bernoulli_ll_grad_matches_fd(self):
        ...
        node = BernoulliInputNode(id=0, scope_var=0, p=p_init)
...
class TestBernoulliQueries:
    def test_cw_leaf_bernoulli(self):
        b1 = BernoulliInputNode(id=0, scope_var=0, p=0.3)
        b2 = BernoulliInputNode(id=1, scope_var=0, p=0.8)
        d = cw_distance(b1, b2)
    ...
    def test_gcw_bernoulli(self):
        b1 = BernoulliInputNode(id=0, scope_var=0, p=0.25)
        b2 = BernoulliInputNode(id=1, scope_var=0, p=0.75)
...
class TestBernoulliSampling:
    def test_empirical_mean(self):
        node = BernoulliInputNode(id=0, scope_var=0, p=0.35)
        draws = Circuit(node).sample(30_000, seed=0)
        assert_allclose(empirical_marginal(draws, 0, 2)[1], 0.35, atol=0.02)

    def test_exact_marginal(self):
        node = BernoulliInputNode(id=0, scope_var=0, p=0.6)
