...
@pytest.mark.parametrize(
    "node",
    [
        BernoulliInputNode(0, 0.4),
        IndicatorInputNode(0, 1, 4),
        LiteralInputNode(0, 0),
        DiscreteLogisticInputNode(0, 1.0, 0.8, 6),
    ],
)
...
@pytest.mark.parametrize(
    "leaf_a, leaf_b",
    [
        (BernoulliInputNode(0, 0.4), BernoulliInputNode(1, 0.6)),
        (IndicatorInputNode(0, 1, 4), IndicatorInputNode(1, 2, 4)),
        (LiteralInputNode(0, 0), LiteralInputNode(1, 1)),
        (
            DiscreteLogisticInputNode(0, 1.0, 0.8, 6),
            DiscreteLogisticInputNode(1, 2.0, 1.0, 6),
        ),
    ],
)
def test_leaf_serializer_roundtrip(leaf_a, leaf_b):
    root = SumNode([ProductNode([leaf_a, leaf_b])], [1.0])
...
def test_new_leaves_participate_in_cw():
    ...
    def build(p0, p1):
        a = BernoulliInputNode(0, p0)
        b = BernoulliInputNode(1, p1)
        root = ProductNode([a, b])
        root.propagate_scope()
        return root
