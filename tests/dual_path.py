...
def _patch_circuit_methods(monkeypatch: Any, wrappers: dict[str, Callable[..., Any]]) -> None:
    def _circuit_likelihood(self: Circuit, data: Any, *args: Any, **kwargs: Any):
        return as_compiled(self).likelihood(data, *args, **kwargs)

    def _circuit_log_likelihood(self: Circuit, data: Any, *args: Any, **kwargs: Any):
        return as_compiled(self).log_likelihood(data, *args, **kwargs)

    def _circuit_sample(self: Circuit, n_samples: int, seed: Any = None, *args: Any, **kwargs: Any):
        return as_compiled(self).sample(n_samples, seed, *args, **kwargs)

    def _circuit_mean_log_likelihood_and_grad(self: Circuit, dataset: Any, *args: Any, **kwargs: Any):
        return as_compiled(self).mean_log_likelihood_and_grad(dataset, *args, **kwargs)

    def _circuit_cw_distance(self: Circuit, other: Any, *args: Any, **kwargs: Any) -> float:
        return as_compiled(self).cw_distance(as_compiled(other), *args, **kwargs)

    def _circuit_cw_distance_and_grad(self: Circuit, other: Any, *args: Any, **kwargs: Any):
        return as_compiled(self).cw_distance_and_grad(as_compiled(other), *args, **kwargs)

    def _circuit_expected_squared_distance(self: Circuit, *args: Any, **kwargs: Any) -> float:
        return as_compiled(self).expected_squared_distance(*args, **kwargs)

    def _circuit_expected_squared_distance_and_grad(self: Circuit, *args: Any, **kwargs: Any):
        return as_compiled(self).expected_squared_distance_and_grad(*args, **kwargs)

    def _circuit_exp_query(self: Circuit, other: Any, *args: Any, **kwargs: Any) -> float:
        return as_compiled(self).exp_query(as_compiled(other), *args, **kwargs)

    def _circuit_exp_query_and_grad(self: Circuit, other: Any, *args: Any, **kwargs: Any):
        return as_compiled(self).exp_query_and_grad(as_compiled(other), *args, **kwargs)

    def _circuit_log_exp_query(self: Circuit, other: Any, *args: Any, **kwargs: Any) -> float:
        return as_compiled(self).log_exp_query(as_compiled(other), *args, **kwargs)

    def _circuit_log_exp_query_and_grad(self: Circuit, other: Any, *args: Any, **kwargs: Any):
        return as_compiled(self).log_exp_query_and_grad(as_compiled(other), *args, **kwargs)

    def _circuit_gcw_crossterm(self: Circuit, other: Any, *args: Any, **kwargs: Any) -> float:
        return as_compiled(self).gcw_crossterm(as_compiled(other), *args, **kwargs)

    def _circuit_gcw_crossterm_and_grad(self: Circuit, other: Any, *args: Any, **kwargs: Any):
        return as_compiled(self).gcw_crossterm_and_grad(as_compiled(other), *args, **kwargs)

    circuit_methods = {
        "likelihood": _circuit_likelihood,
        "log_likelihood": _circuit_log_likelihood,
        "sample": _circuit_sample,
        "mean_log_likelihood_and_grad": _circuit_mean_log_likelihood_and_grad,
        "cw_distance": _circuit_cw_distance,
        "cw_distance_and_grad": _circuit_cw_distance_and_grad,
        "expected_squared_distance": _circuit_expected_squared_distance,
        "expected_squared_distance_and_grad": _circuit_expected_squared_distance_and_grad,
        "exp_query": _circuit_exp_query,
        "exp_query_and_grad": _circuit_exp_query_and_grad,
        "log_exp_query": _circuit_log_exp_query,
        "log_exp_query_and_grad": _circuit_log_exp_query_and_grad,
        "gcw_crossterm": _circuit_gcw_crossterm,
        "gcw_crossterm_and_grad": _circuit_gcw_crossterm_and_grad,
    }
    for name, fn in circuit_methods.items():
        monkeypatch.setattr(Circuit, name, fn, raising=False)
...
def install_compiled_routing(monkeypatch: Any, orig: dict[str, Callable[..., Any]]) -> None:
    ...
    _patch_circuit_methods(monkeypatch, wrappers)
...
class CircuitBackend:
    ...
    def prepare(self, obj: Any) -> Any:
        if not self.compiled:
            if isinstance(obj, (Circuit, CompiledCircuit)):
                return obj
            if isinstance(obj, CircuitNode):
                return Circuit(obj)
            return obj
