...
from sparc import CategoricalInputNode, GradBundle, ProductNode
...
def test_refresh_parameters_updates_batched_ll():
    ...
    root = ProductNode(children=[leaf])
    circuit = root
    ...
    grads = GradBundle()
    grads.cat_grads = {int(leaf.id): np.array([0.1, -0.1])}
    apply_grads(circuit, grads, lr=0.5)
